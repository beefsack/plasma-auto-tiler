...
// the id sets here; this planner only decides which id may be removed. An
// entry is removable only when it is empty (no occupying windows), invisible
// on every output, and not the last global desktop. The ordered snapshot is
// the single determinism source: repeated calls with fresh equivalent
// snapshots select the same earliest eligible entry in stable order, never a
// set-member tie-break.
...
// when the snapshot holds a single global desktop or no entry is empty and
// invisible; the caller then leaves the desktop list unchanged.
