...
describe("background empty-domain retirement", () => {
    it("closing the final hidden member retires the baseline and frees the slot", () => {
        ...
        assert.equal(activeSets.length, 0, "hidden empty reconcile never writes native focus");
        ...
        fire("geometry");
        runDebounce(timers);
        assert.equal(calls.length, answered, "retired empty domain sends nothing further");
        assert.equal(activeSets.length, 0);
        ...
        assert.deepEqual(
            (admit["windows"] as Array<Record<string, unknown>>).map((entry) => entry["window"]),
            ["win-n"],
            "fresh hidden domain carries complete observation",
        );
        assert.equal(activeSets.length, 0);
    ...
    it("multi-member collapse is not falsely committed", () => {
        ...
        const seeded = calls.map((call) => payloadAt(calls, calls.indexOf(call)));
        const hiddenSeed = seeded.find(
            (payload) => (payload["domain"] as Record<string, unknown>)["workspace"] === "ws-2",
        );
        assert.deepEqual(
            ((hiddenSeed?.["windows"] as Array<Record<string, unknown>>).map((entry) => entry["window"])).sort(),
            ["win-h1", "win-h2"],
            "hidden domain seeds with complete two-member observation",
        );
