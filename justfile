...
dev-on:
    ...
    if [[ -f "$PID_FILE" && -f "$RECEIPT_PTR" ]]; then
      ...
      RECORDED_RECEIPT="$(cat "$RECEIPT_PTR")"
      if [[ "$RECORDED_PID" =~ ^[0-9]+$ ]] && [[ -d "/proc/$RECORDED_PID" ]] && [[ -f "$RECORDED_RECEIPT" && ! -L "$RECORDED_RECEIPT" ]]; then
        CURRENT_EXE="$(readlink "/proc/$RECORDED_PID/exe" 2>/dev/null || true)"
        RECORDED_EXE=""
        [[ -f "$EXE_FILE" ]] && RECORDED_EXE="$(cat "$EXE_FILE")"
        RECORDED_START=""
        ...
        if [[ -n "$CURRENT_EXE" && "$CURRENT_EXE" == "$BIN" && "$CURRENT_EXE" == "$RECORDED_EXE" && -n "$CURRENT_START" && "$CURRENT_START" == "$RECORDED_START" ]]; then
          ...
          if command -v jq >/dev/null 2>&1; then
            ...
            [[ -n "$SID" ]] && echo "planner pid: $RECORDED_PID, script id: $SID"
          fi
          exit 0
        fi
      fi
    fi
    # Live guard: a worktree Planner already owning the D-Bus name means dev
    # mode is up even without state files. Report and make no changes.
    if busctl --user call org.freedesktop.DBus /org/freedesktop/DBus org.freedesktop.DBus GetNameOwner s "$PLANNER_BUS" >/dev/null 2>&1; then
      OWNER="$(busctl --user call org.freedesktop.DBus /org/freedesktop/DBus org.freedesktop.DBus GetNameOwner s "$PLANNER_BUS" 2>/dev/null || true)"
...
dev-off:
    ...
    [[ -f "$EXE_FILE" ]] || { echo "error: no recorded planner exe ($EXE_FILE missing)" >&2; exit 1; }
    [[ -f "$START_FILE" ]] || { echo "error: no recorded planner start identity ($START_FILE missing); refusing ambiguous kill" >&2; exit 1; }
    RECORDED_PID="$(cat "$PID_FILE")"
    RECORDED_EXE="$(cat "$EXE_FILE")"
    RECORDED_START="$(cat "$START_FILE")"
    ...
    # Dynamically find the controller receipt: pointer file first, else glob.
    RECEIPT=""
    if [[ -f "$RECEIPT_PTR" ]]; then
      ...
      if [[ "${#LIVE[@]}" -eq 1 ]]; then
        ...
        echo "error: no controller receipt found under $RUNTIME_DIR (and $RECEIPT_PTR missing); refusing ambiguous teardown" >&2
        exit 1
      else
    ...
    # Pass both the receipt (env) and the ID (arg) to start-test.sh stop.
