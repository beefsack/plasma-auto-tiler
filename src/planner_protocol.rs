...
mod tests {
    ...
    fn retained_pointer_resize_reflows_allocation_and_shares() {
        // Retained `op=pointer-resize` routes through
        // `Session::propose_pointer_resize` (pointer-only capability) with
        // acknowledge + `ResizePostObservation` fencing, like keyboard
        // resize. Landscape 1200px output splits 600/600, so dragging the
        // focused right leaf's left edge to 550 must move shares/geometry.
        ...
        assert_eq!(old.len(), new.len(), "{reply}");
        assert_ne!(old, new, "{reply}");
        let before: Vec<(i32, i32, i32, i32)> = vec![(0, 0, 600, 800), (600, 0, 600, 800)];
        assert_ne!(geometry_rects(&reply), before, "{reply}");
        assert_eq!(planner.retained_domains(), 1);
        // Fenced commit: a follow-up directional command still plans.
        let follow = retained_request(
            "ptr-ok-2",
            "owner-1",
            "gen-1",
    ...
    fn retained_pointer_resize_out_of_bounds_refuses_precisely() {
        // Boundary outside the domain work-area extent (1200px wide) fails
        // closed with exactly one refusal reason, no detail echo.
        let mut planner = seed_pointer_planner();
        let request = retained_request(
            "ptr-bad-1",
            "owner-1",
