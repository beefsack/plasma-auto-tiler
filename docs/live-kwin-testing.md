...
| Desktop 2 incident (`workspace-management-fixes` unit-09, succession 6): a Lead re-enabled the plugin on the user's live host acting on an inherited, unverified dispatch-brief claim about which desktops held windows, instead of taking its own fresh occupancy baseline immediately before the live enable. The startup sweep removed 10 desktops including one, "Desktop 2", the Lead could not distinguish as correctly-empty-at-removal-time versus wrongly-removed-while-populated. The plugin was disabled immediately on discovering the missing desktop; no further live mutation was attempted; the incident was escalated rather than continued. Later journal-evidence triage (succession 8) traced every one of the 10 removals to its own freshly-read, fully-decoded live occupancy check that same iteration (zero `workspace-cleanup-deferred:window-occupancy-unknown` events), proving the removal logic itself was not defective; the user separately confirmed directly that all his windows were intact and nothing was lost. **Ruling: the code correctly saw Desktop 2 as empty at removal time; the gap was the Lead's own stale, uncross-checked baseline, not a code defect.** | Never act on an inherited occupancy/state claim from a dispatch brief for a live-mutating action; always take a fresh, direct, non-inherited baseline of every affected object immediately before the mutation, in the same stint that performs it. This is now a standing procedure for any future live enable of this project's cleanup path (see `workspace-management-fixes/state.md`'s "Next action" precedent, itself now archived). |
