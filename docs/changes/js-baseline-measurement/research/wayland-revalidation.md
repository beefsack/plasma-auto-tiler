...
Not attempted. The brief's optional stretch (a config-gated
...
`runAmplifiedReconcile` in `variant-b.js` plus a live load/run. This worker
reached its ~30-tool-call budget ceiling on the required work (precondition
verification, smoke test, two full tiers, reversal verification), so the
stretch was left for a successor rather than risk an incomplete or
unverified calibration. **Left for successor.** No script file was modified
by this unit.
