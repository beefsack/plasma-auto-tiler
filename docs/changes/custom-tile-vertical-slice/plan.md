...
| unit-21 | Accepted nested yield-primitive evidence 2026-08-12 | Prove safe exactly-once event-loop-turn yield primitives for remove -> yield -> split and add session-local managed-scope ratio-free dwindle ownership with guarded reconstruction. | unit-19, unit-20 | controller, entry, `kwin-globals.d.ts` (`callDBus` declaration), controller tests, isolated nested-compositor probe | No | Zero-delay `QTimer` one-shot and `callDBus(...ListNames, cb)` both PROVEN exactly-once real event-loop-turn yields (21/21 chained, 8/8 Design-B cycles); `Qt.callLater` absent; `desktopsChanged` fires for desktop changes rather than one callback per arm and is therefore not a safe yield seam; deleteLater ordering established (timer before DeferredDelete, callDBus after; safety from fresh re-resolution, not ordering). Ownership: takeover adopts/rebuilds ratio-free dwindle, selected-overlay precedence, per-scope inert isolation, one guarded `remove()` + fresh whole-root decode for a freed leaf, adds/removes never mixed, no timer for structural transitions. Typecheck/build pass, 417 tests (421 after the lost-callback re-drive correction) across 49 suites, 248 lifecycle checks. |
| unit-23 | Accepted static; partially live-proven 2026-08-12 | Replace the signal-driven reconstruction seam with a named one-shot event-loop-yield pipeline and live-validate production removal rebuild in an isolated nested compositor. | unit-21 | controller, entry, `kwin-globals.d.ts`, controller tests, isolated nested-compositor probe | Yes, isolated nested | `ControllerEnvironment.yieldOnce(callback)` via the proven `callDBus(...ListNames, cb)` primitive; two-phase non-timer pipeline (synchronous removals-only collapse with fresh whole-root decode after every removal, then synchronous splits-only rebuild) with per-pending phase/arm bookkeeping; a lost split-phase yield is re-driven by a later lifecycle event because a D-Bus error reply is logged and never invokes the callback (`scripting.cpp:361-364`); a failed arm marks the scope inert. `unit-23/attempt-02`: 4 -> 3 and 3 -> 2 removals each observed `removed-obs tile=set` then the full deferred-collapse chain and fresh trees `H[A,V[C,D]]` / `H[C,D]` with survivors tiled once; timings 6713 us (4 -> 3) and 5172 us (3 -> 2), cleanup closes 175/900 us, n=2 samples, not frame-time/visual-jank/client-geometry; stopped at the rect locator missing center x=479 before N=1/N=0 snapshots and detached-window close (harness defect, not a product defect). Typecheck pass, 429 tests / 49 suites, 248 lifecycle checks; bundle `88844452...`. |
...
- [x] unit-21 Event-loop yield primitives and automatic dwindle ownership (nested-proven, 2026-08-12)
- [x] unit-23 Guaranteed deferred dwindle reconstruction and nested removal-rebuild validation (static accepted; `unit-23/attempt-02` partial live, 2026-08-12)
...
- Resolved by `unit-21` nested evidence 2026-08-12: the only safe one-shot event-loop-turn yield primitives for the remove -> yield -> split sequence are a zero-delay `QTimer` one-shot and the `callDBus(...ListNames, cb)` async reply seam. `Qt.callLater` is absent and the `desktopsChanged` re-entering signal is ruled out; deleteLater ordering does not matter because safety comes from fresh re-resolution, never from a timer settle barrier.
- Resolved by `unit-23` static and nested evidence 2026-08-12: automatic dwindle reconstruction is a two-phase non-timer pipeline (synchronous removals-only collapse, then synchronous splits-only rebuild) separated by a named one-shot `yieldOnce` event-loop yield. Because a D-Bus error reply is logged and never invokes the armed callback (`scripting.cpp:361-364`), a lost split-phase reply is re-driven by a later ordinary lifecycle event, at most twice per phase; then only that scope becomes inert instead of stranding. `unit-23/attempt-02` live-proved the `callDBus` deferred-collapse correction (4 -> 3 and 3 -> 2 removal rebuilds) but stopped at the rect locator harness defect before authoritative N=1/N=0 shape snapshots and detached-window close; those cases remain unknown.
- Reserved to the user: stable multi-output identity, persistence
  ...
  helper-component language/runtime. The authorized automatic-ownership trial
  is restricted to `eDP-1`; no broader output policy is decided.
...
a dev-only, schema-pinned dwindle `[Tiling]` generator/validator and
exact group-selection tests (pure, no KWin). Note: the proposed `unit-21`
ID was already consumed by the 2026-08-12 yield-primitive/dwindle-ownership
work; assign a fresh unit ID when dispatching. It must not claim runtime
