...
- Live-runner `/Effects` correction: a second user retry briefly launched
  nested KWin but failed at `/Effects`. Static diagnosis found single-shot
  readiness, conflated query/support state, and an undiscoverable evidence
  root; later independent review found incorrect uppercase
  `org.kde.KWin.Effects`. The accepted correction adds bounded `/Effects`
  readiness/liveness with retained status/stdout/stderr/attempt evidence,
  immediate evidence-root output, source-aligned `isEffectSupported` and
  `loadEffect` booleans, exact lowercase `org.kde.kwin.Effects`, a strict fake
  interface guard, and focused lifecycle tests. Both syntax checks passed. The
