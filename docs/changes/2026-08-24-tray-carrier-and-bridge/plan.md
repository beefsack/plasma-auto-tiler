...
| KWin publisher has focused, typecheck, build, and isolated broad evidence. | `gate.tray-publisher-focused`, `gate.tray-publisher-typecheck`, `gate.tray-publisher-build`, `gate.tray-publisher-broad` | See Production Gate Map. | All production gate baselines pass on the same scoped snapshot. | Static production/lifecycle work is accepted through `2a626a9`; immediate publisher work is accepted through `69ad080`. |
...
- Static production/lifecycle work is accepted on main through `2a626a9`; immediate publisher work is accepted through `69ad080`.
- The historical 05a direct-helper result was 75/0/0 on an old candidate. Current-main live helper validation remains pending after lifecycle changes.
- The reconstructed 05a static harness was accepted at 95/0/0 but is uncommitted because the installer gate was environment blocked. It is not main evidence.
- 05b KWin live acceptance is unaccepted: the earlier reported run had no attributable evidence, and later preflight stopped before mutation because of a KWin identity mismatch. Static evidence and identity corrections remain only in the preserved candidate, not main. No positive 05b KWin-origin delivery claim exists.
- Panel rendering, physical/manual checks, and login-autostart remain separate manual release gates.

## Final Outcome

- Static production/lifecycle and immediate-publisher work is accepted on main
  through `2a626a9` and `69ad080`. 05a live helper validation and 05b KWin live
  acceptance remain unaccepted; panel rendering, physical/manual checks, and
  login-autostart remain separate manual release gates.
