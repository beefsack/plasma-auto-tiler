...
surface reported by the assigned Worker; they are useful for scoping a future
prototype but are not runtime proof.

The Worker returned without file changes but violated its mandatory call budget
(27 own calls rather than returning by call 16 and no later than call 20). Its
report is therefore not accepted as standalone evidence. No replacement was
authorized because this change permits exactly one Worker. The conclusions here
are deliberately limited to the independently documented API boundary and the
reported code locations that a future implementation must revalidate.
...
The prior [window visual-effects feasibility](../../2026-08-14-window-visual-effects-feasibility/research/feasibility.md)
...
2. [Window visual-effects feasibility](../../2026-08-14-window-visual-effects-feasibility/research/feasibility.md) - prior project research establishing the unresolved rendering-carrier boundary for a header.
