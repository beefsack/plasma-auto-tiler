...
- No go files, no chat-ready signals, and no user/coordinator manual external
  ...
  indefinite external wait. The supervisor waits only for an exact matching proof
...
`kwin_wayland`; report for a human decision. If the
...
The operator, not the harness, tells the user the manual action
...
- Before dispatching M1, the operator announces, verbatim:
...
- Before dispatching M2, the operator announces, verbatim:
...
3. Two manual live stages (M1, M2), each a separate 90-second command. The
   operator tells you the exact action immediately before each stage:
