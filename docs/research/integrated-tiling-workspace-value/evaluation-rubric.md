...
structural-authoring/direct-placement interaction reference. It has no baseline
configuration, no companion standard, and no journey cells in this rubric. Its
bounded evidence is retained in the [reference comparison](../../reference-wm-comparison.md)
using the section 10 template with a distinct evidence prefix (e.g., B-01). The
...
- (d) Bounded set: the companion set is capped (default cap: 3 components),
  ...
  beyond the cap requires explicit approval during unit-02. Components
...
structural reference is NOT a row of this comparator matrix; its bounded
evidence is in the [reference comparison](../../reference-wm-comparison.md)
...
Future research creates one record per matrix cell per comparator.
...
8 classifications with established comparator prefixes. The bspwm structural
reference is recorded in the [reference comparison](../../reference-wm-comparison.md)
with a distinct prefix (e.g., B-01), scoped by section 2.3.
...
| U8 | Whether a safe hands-on environment exists for unit-04 | Decision under the plan safety gate |
