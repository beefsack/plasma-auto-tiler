...
Live-session empirical probe. A one-shot KWin script (`clock-probe.js`)
...
Direct consequence for the variant-script capture procedure and the README: the
variant scripts connect their handlers at top level, so the documented load-only
commands are INCOMPLETE. The harness must call `start()` once
...
```
node --check clock-probe.js
```
Outcome: exit 0 (syntax valid; does not validate KWin globals).

```
grep -Pn '[\x{2014}\x{2018}\x{2019}\x{201C}\x{201D}\x{2026}\x{00A0}]' clock-probe.js
```
...
- `start()`-after-`loadScript()` requirement is a change to the documented
  load procedure must use load-once + start-once.
