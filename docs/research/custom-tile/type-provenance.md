...
The correction review removed the rejected package/import, corrected the
explicit esbuild test entry, and authored the local subset without prohibited
TypeScript escape hatches. Review rejected this attempt before installation:
...
guard. These narrow corrections require another correction pass because the
attempt's single correction round is exhausted.
